...
document.addEventListener('DOMContentLoaded', () => {
    ...
    examTabs.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (targetList) {
                ...
                // Optional: Scroll the list into view if needed (can be jumpy)
                // setTimeout(() => { // Delay scroll slightly for smoother transition after display change
    ...
    if (examTabs.length > 0) {
         ...
         if (!initiallyActive) {
            // Check if there's content to show for the first tab before clicking
            ...
            if (firstSubjectName) {
                 ...
                 if (document.getElementById(firstListId)) {
                     examTabs[0].click(); // Simulate a click only if content exists
                 }
            }
         } else {
            // If one is already active in HTML, ensure its content is shown correctly
